	...
	function(root) {
		...
		validate.hasError = function(field, options) {
			...
			if (validity.rangeOverflow)
				return localSettings.messageRangeOverflow.replace(
					'{max}',
			...
			if (validity.rangeUnderflow)
				return localSettings.messageRangeUnderflow.replace(
					'{min}',
		...
		validate.showError = function(field, error, options) {
			...
			var message = field.form.querySelector(
				'.' + localSettings.errorClass + '#error-for-' + id
			)
			if (!message) {
		...
		var blurHandler = function(event) {
			...
			if (!event.target.form || !event.target.form.matches(settings.selector))
				return
